...
<div align="center"> <img src="src/mom_notes/web/static/icons/mark.svg" alt="MOM Notes logo" width="88"> <h1>MOM Notes</h1> <p><strong>Private AI meeting notes, local transcription, and speaker diarization.</strong></p> <p>Record, transcribe, identify speakers, and create structured Minutes of Meeting (MOM) on your own computer.</p>

<p> <a href="#features">Features</a> · <a href="#installation">Install</a> · <a href="docs/README.md">Documentation</a> </p>

<p> <img alt="Python 3.11+" src="https://img.shields.io/badge/Python-3.11%2B-3776AB?logo=python&logoColor=white"> <img alt="Platforms" src="https://img.shields.io/badge/Windows%20%7C%20macOS%20%7C%20Linux-supported-0D9488"> <img alt="Local first" src="https://img.shields.io/badge/AI-local--first-0D9488"> <img alt="Theme" src="https://img.shields.io/badge/Theme-White%20%26%20Teal%20Green-0D9488"> <img alt="License MIT" src="https://img.shields.io/badge/license-MIT-111827"> </p> </div>

MOM Notes is an open-source, self-hosted AI meeting assistant for Windows,
...
sides of the conversation. See the audio capture setup guide
for Windows loopback, Linux monitors, and macOS virtual inputs.

MOM Notes is a private, local-first alternative to commercial AI meeting
...
MOM Notes 0.6: the complete meeting, available to your AI
...
The 0.6 series also turns MOM Notes into a private knowledge source for the desktop
...
clients to completed meetings without copying the MOM Notes database or
...
the existing MOM Notes virtual environment. That process talks only to the
...
and multiple clients can safely use the same running MOM Notes instance.
...
or for Codex/ChatGPT Desktop. See the Local MCP server guide for
...
GPU name, VRAM when available, and the MOM Notes GPU process.
...
Download [install-update.bat](https://github.com/estebanstifli/MOM Notes/raw/main/install-update.bat).

Double-click the downloaded file.

Wait for setup to finish, then open the new MOM Notes folder and
double-click start.bat.

Open http://127.0.0.1:8765 in your browser.

The bootstrap installer checks for Git and Python 3.11 or newer, installs
missing prerequisites for the current Windows user, clones MOM Notes, and
...
MOM Notes folder beside the downloaded .bat. For example, a file saved as
C:\Users\Name\Downloads\install-update.bat installs the application in
C:\Users\Name\Downloads\MOM Notes. Move the .bat to another writable
...
from the official MOM Notes repository or mom-notes.local.

Pinokio: one-click local installation
MOM Notes can also be installed through Pinokio,
...
In Pinokio, choose the option to install an app from a Git repository.

Enter https://github.com/estebanstifli/MOM Notes.git.

Select Install MOM Notes, wait for the model downloads to finish, then
select Start MOM Notes.

Use Open MOM Notes in Pinokio to open the local web interface.
...
remain managed by MOM Notes and are not removed automatically.

macOS and Linux
git clone https://github.com/estebanstifli/MOM Notes.git
cd MOM Notes
chmod +x install.sh
./install.sh --ai-backend cpu
.venv/bin/mom-notes --no-browser
...
the webhook integration guide for events, payloads,
...
Create a read token, add it to .env, restart MOM Notes, and install the model
from Settings:

MOM_NOTES_PYANNOTE_TOKEN=hf_your_read_token_here
...
default. See the Live AI Assistant guide.
...
adding another toolbar row. MOM Notes warns before unsaved edits are discarded
...
MOM Notes includes a read-only local MCP server for Claude Desktop, ChatGPT
Desktop, Codex, VS Code, Cursor, and other clients that support stdio. Each
client launches the Python module from the existing MOM Notes virtual
environment; no separate executable is distributed. The lightweight MCP
processes all connect to the one running MOM Notes instance, which remains the
...
operations. MOM Notes also generates the exact Claude Desktop JSON and
Codex/ChatGPT Desktop TOML snippets from Settings, including the correct local
Python path. See Local MCP server for Windows, Linux, and macOS
configuration examples.

Help translate MOM Notes
MOM Notes welcomes community-maintained UI localizations. The source catalogue
is src/mom_notes/web/static/locales/en.json;
...
terms such as MOM Notes, RAG, Faster Whisper, Word, and Markdown.
...
Plugins are discovered through the standard mom_notes.plugins package entry
...
repository. Authors do not need to merge plugin code into MOM Notes: when it is
...
may then add it to community-plugins.json. Listing is
discretionary and is not a security audit or endorsement.

The catalog is currently an informational JSON file; MOM Notes does not fetch
...
permissions, and enable it. See the Plugin API and installation guide,
plugin/provider development guide,
documentation index, and public roadmap.

Advanced installation from source
The installers create an isolated .venv inside the repository. MOM Notes
...
.venv/bin/mom-notes --no-browser
If MOM Notes was initially installed in CPU mode and the user later selects a
...
.\install.ps1 -ModelsDirectory "D:\MOM Notes\Models"
...
.\.venv\Scripts\python.exe -m mom_notes --no-browser
.\.venv\Scripts\mom-notes.exe --help
MOM Notes binds to 127.0.0.1 by default and is not exposed to the network
...
new defaults without overwriting stored values. See Safe updates
...
.\.venv\Scripts\mom-notes-models.exe --models all
.\.venv\Scripts\mom-notes-models.exe --models whisper --whisper-model medium
.\.venv\Scripts\mom-notes-models.exe --models diarization summary
.\.venv\Scripts\mom-notes-models.exe --models embeddings
.\.venv\Scripts\mom-notes-models.exe --models nvidia-parakeet
.\.venv\Scripts\mom-notes-models.exe --models nvidia-nemotron
...
start. They can also be overridden with MOM_NOTES_DATA_DIR, MOM_NOTES_MODELS_DIR,
--data-dir, or --models-dir.

Recording and post-processing
After stopping a recording or importing a media file, MOM Notes presents the
processing choices before starting expensive work:

Run or skip speaker diarization.

Detect the number of speakers automatically or provide the known count.

Run or skip the selected final transcription pass.

Run or skip AI analysis using the selected note format.
...
Start with the documentation index. Read
Architecture, Contributing, and the
Roadmap before extending an engine or submitting core changes.
...
License
MOM Notes is released under the MIT License.
