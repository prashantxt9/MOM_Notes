...
def _configure_cuda_dlls() -> None:
    ...
    windows_os: Any = os
    if hasattr(windows_os, "add_dll_directory"):
        for directory in directories:
            try:
                _CUDA_DLL_HANDLES.append(windows_os.add_dll_directory(str(directory)))
            except Exception:
                pass
...
class FasterWhisperEngine:
    ...
    def _transcribe_sync(
        ...
        try:
            ...
            with slots:
                try:
                    raw_segments, info = _iterate_transcribe(model)
                    segment_iterator = iter(raw_segments)
                    first_segment = next(segment_iterator, None)
                except Exception as error:
                    err_str = str(error).casefold()
                    if request.device in ("auto", "cuda") and any(
                        k in err_str for k in ("cublas", "cudnn", "cuda", "library", "dll", "out of memory")
                    ):
                        logger.warning(
                            "Faster Whisper CUDA transcription failed (%s). Retrying on CPU.",
                            error,
                        )
                        cpu_model, cpu_slots, cpu_key = self._get_model(
                            model_class,
                            model=request.model,
                            device="cpu",
                            device_index=0,
                ...
                def _all_segments():
                    if first_segment is not None:
                        yield first_segment
                    yield from segment_iterator

                for index, segment in enumerate(_all_segments()):
                    if is_cancelled():
                        raise JobCancelledError("Transcription was cancelled")
        ...
        *,
        ...
        with self._model_lock:
            ...
            try:
                ...
                err_str = str(error).casefold()
                if device in ("auto", "cuda") and any(
                    k in err_str for k in ("cublas", "cudnn", "cuda", "library", "dll", "out of memory")
                ):
                    logger.warning(
                        "Faster Whisper failed on %s (%s). Falling back to CPU with int8.",
                        device,
                        error,
                    )
                    fallback_compute = (
                    ...
                    self._models[fallback_key] = model_instance
                    self._model_slots[fallback_key] = threading.BoundedSemaphore(num_workers)
                    self._resident_models = (model,)
                    logger.info("Faster Whisper model %s fallback on CPU is resident in memory", model)
                    return model_instance, self._model_slots[fallback_key], fallback_key
                raise
